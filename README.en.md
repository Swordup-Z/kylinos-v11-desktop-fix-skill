...
When an ordinary user produces new reusable repair experience, do not directly modify or commit $HOME/.os-fix-skill; create a local patch under $HOME/.os-fix-skill-patches/ so the skill repository can continue to use git pull --ff-only. Developers with repository write access should maintain the repository according to their current development environment's global prompts or project rules.
```
