...
- Local source customizations may be committed in the local worktree to preserve a clear change boundary, but they should not be pushed by default. Export the commit as a patch under `/data/usershare/kylinos-local-sources/<component-or-fix>/patches/`, which belongs to the local customization project root and sits alongside `<source-tree>/`, `build/`, and `rollback/`; record the commit, patch path, baseline version, install status, and rollback path in `CUSTOMIZATION.md` so the customization can be reapplied or migrated after system package upgrades.
