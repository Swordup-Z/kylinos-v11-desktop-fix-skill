...
When an ordinary user produces new reusable repair experience, do not directly modify or commit $HOME/.os-fix-skill; create a local patch under $HOME/.os-fix-skill-patches/ so the skill repository can continue to use git pull --ff-only. Maintain the repository directly only after explicitly entering maintainer mode, confirming osSkill.maintainer=true for $HOME/.os-fix-skill, and passing a dry-run push.
```
