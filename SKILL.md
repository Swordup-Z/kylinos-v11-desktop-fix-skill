...
- 新增经验默认走 `$HOME/.os-fix-skill-patches/` 本地 patch，保持 skill 仓库干净以便拉取上游；只有显式进入开发者维护模式、确认 `osSkill.maintainer=true` 且 dry-run push 通过时，才直接更新最小匹配场景的 `references/` 或 `knowledge/<scenario>/` 章节，并按当前开发环境规则提交和维护仓库。维护模式、安全边界、通用诊断闭环属于 `knowledge/system/`。
...
- 每次问题确认修复完成后，必须主动判断是否产生可复用经验。默认把建议变更保存为本地 patch 并在最终回复中说明 patch 路径；开发者维护模式下才把经验写入对应 `references/` 路由或 `knowledge/` 章节。只有入口边界、触发范围或路由结构变化时，才更新 `SKILL.md`。
- 如果实际解决的是当前 skill 尚未覆盖的 KylinOS Desktop V11 系统修复问题，默认生成包含新增 `references/<scenario>.md`、同场景 `knowledge/` 章节和必要 `SKILL.md` 路由变更的 patch；开发者维护模式下可直接落库。
